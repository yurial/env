...
- `yurial/` — dotfiles пользователя `yurial`:
  ...
  - systemd: `.config/systemd/user/opencode-server.service` — user-сервис `opencode serve` (порт 4096, только localhost; `systemctl --user status opencode-server`; перезапуск — alias `opencode-restart`; автостарт при загрузке — при включённом `loginctl enable-linger`)
...
- `skills/` — специализированные навыки: `coding` (правила написания и ревью кода на любом языке; собственная спека навыка — `SPEC.md`), `spec` (спеки, spec-first flow; механические проверки спек — скрипт `speclint` с тест-набором `tests/` в каталоге навыка; собственная спека навыка — `SPEC.md`), `tla-plus` (TLA+-спеки и TLC-модели), `tlaps` (иерархические доказательства через tlapm), `tlc-run` (запуск TLC и разбор вывода), `glab` (работа с GitLab через CLI glab: MR, issues, CI, REST API; собственная спека навыка — `SPEC.md`).
