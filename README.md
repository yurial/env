...
- `opencode.json` — единственный провайдер `vk-zai-personal` (модели `heavy`=glm-5.3, `flash`=glm-5.3-flash, `cheap`=glm-4.6v; у `heavy` и `flash` варианты reasoning `low`/`high`/`max`; у `cheap` вариант `stupid` с выключенным thinking `"thinking":{"type":"disabled"}` — glm-5.3 выключение thinking не поддерживает, glm-4.6v поддерживает). Все прочие встроенные провайдеры (openai, gemini, openrouter, opencode, anthropic и т.д.) отключены через `disabled_providers`. `default_agent: main`. Bash-команды разрешены все, кроме `sleep*`. Телеметрия/шаринг глобально отключены (`"share": "disabled"`).
